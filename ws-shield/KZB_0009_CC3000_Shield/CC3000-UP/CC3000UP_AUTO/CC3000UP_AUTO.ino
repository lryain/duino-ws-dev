...
//@TONYLABS CC3000 Shield Default Pins
...
void UseDefaults(void) {
	...
	for (int i=0; i<MAC_SIZE; i++) {
  if(i==0){
		EEPROM.write(MAC_ADDRESS_EEPROM_LOCATION+i, 0);
  }else{
...
void DisplayMenu(void) {
	...
	Serial.println(F("1.0--1.24"));
	Serial.println(F("4Y,6Y,7Y,8Y,9Y,0"));
...
{
  Serial.begin(115200);
  Serial.println(F("The DIUINOPEAK CC3000 Firmware 1.24 Upgrader"));
  Serial.println(F("-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-"));
  BasicCC30000Test();
  delay(2000);
  GetCC3000Info(true);
