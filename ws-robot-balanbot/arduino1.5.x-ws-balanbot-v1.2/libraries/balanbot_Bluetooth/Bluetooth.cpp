 ...
 */
#include "Bluetooth.h"
#include "tools.h"//调用tools中的setValues()函数
#include "encoder.h"//调用getposition()函数
#include <SPP.h>//用于下面的SerialBT
...
void BluetoothN::steer(Command command)
{
	encoder encoderB;//新建一个encoder类对象，用于后面的getPosition()函数
  ...
  if(command==stop)
  {
	  ...
	  {
		  targetPosition=encoderB.getWheelsPosition();
		  stopped=false;
	  }
  }
  lastCommand=command;
...
double BluetoothN::scale(double input,double inputMin,double inputMax,double outputMin,double outputMax)
{
    double output;
...
#ifdef ENABLE_SPP
void BluetoothN::readSPPData()
{
	Tool toolB;//新建一个Tool对象，用于下面的setValues(dataInput)函数
	//SPP SerialBTB;//新建一个SPP类对象，用于下面的函数调用
	if(SerialBT.connected)//the spp connection won't return data as fast as the controllers,so we will handle it separately
	{
		//Serial.println("Yeah!bluetooth connected!");//测试连接用

	    if(SerialBT.available())
	    {
	    	//Serial.println("bluetooth available!");//测试手机上的数据流是否可以发送
	    	uint8_t i=0;
	    	while(1)
	    	{
	    		dataInput[i]=SerialBT.read();//蓝牙数据流在从手机流向小车
	    	...
	    	bluetoothData=true;
	    	toolB.setValues(dataInput);
	    }
	}
}
#endif

void BluetoothN::readUsb()
{
//#ifdef ENABLE_USB
	Usb.Task();//The Spp data is actually not send  until this is called,one could call SerialBT.send() directly as well
	if(Usb.getUsbTaskState()==USB_STATE_ERROR&&layingDown)
	{
	    Serial.println("USB fail");
...
//#endif
    ...
    else if (Wii.wiiUProControllerConnected && !commandSent)
    { // The Wii U Pro Controller Joysticks has an range from approximately 800-3200
...
#endif // ENABLE_WII
#ifdef ENABLE_XBOX
    if (Xbox.Xbox360Connected[0] && !commandSent)
    { // We will only read from the first controller, up to four is supported by one receiver
