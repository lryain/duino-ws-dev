...
This is an example of how to read data from a Dynamixel and write its value via a second Serial_AX port on an Arduino Mega.
...
void setup(){
  Serial_AX.begin(57600);                                       // Set up Software Serial
...
Serial_AX.println(Temp);
    if (Temp >> 12 == 0){               // There was no error found - Print value
        Serial_AX.print("Servo ID: ");Serial_AX.print(SERVO_ID);Serial_AX.print(" Temperature is ");Serial_AX.println(Temp);
...
unsigned int Volt = Dynamixel.readVoltage(SERVO_ID);
    if (Volt >> 12 == 0){            // There was no error found - Print value
        Serial_AX.print("Servo ID: ");Serial_AX.print(SERVO_ID);Serial_AX.print(" Voltage is ");Serial_AX.println(Volt * 0.1);
    }else{                            // There was an error found - Print the error
...
unsigned int   Speed = Dynamixel.readSpeed(SERVO_ID);
    ...
    if (Speed >> 12 == 0){              // There was no error found -  Print value
        Serial_AX.print("Servo ID: ");Serial_AX.print(SERVO_ID);Serial_AX.print(" speed is rpm ");Serial_AX.println(Speed * 0.11);
    }else{                              // There was an error found - Print the error
...
unsigned int   Pos = Dynamixel.readPosition(SERVO_ID);
    ...
    if (Pos >> 12 == 0){              // There was no error found - Print value
        Serial_AX.print("Servo ID: ");Serial_AX.print(SERVO_ID);Serial_AX.print(" Position is Deg ");Serial_AX.println(Pos * 0.088);
    }else{                            // There was an error found - Print the error
        readStatusPacketError(Speed);
    }
...
unsigned int Load = Dynamixel.readLoad(SERVO_ID);
    ...
    if (Load >> 12 == 0){              // There was no error found - Print value
        if (Load >> 10 == 0){        // Load direction is CCW
           Serial_AX.print("Servo ID: ");Serial_AX.print(SERVO_ID);Serial_AX.print(" Load is Counter Clockwise at % ");Serial_AX.println((Load & 0x3FF) *0.1 );
        }else if (Load >> 10 == 1){  // Load direction is CW
          Serial_AX.print("Servo ID: ");Serial_AX.print(SERVO_ID);Serial_AX.print(" Load is Clockwise at % ");Serial_AX.println((Load & 0x3FF) * 0.1);
        }else{
          Serial_AX.println("Process error");
        }
...
void readStatusPacketError( byte error){    // We are using byte becasue we are only using the lower 8 bits, if you use a "unsigned int" you will need to mask it e.g (error & 0xFF)
  ...
  switch (error){
      case VOLTAGE_ERROR:
      Serial_AX.print("Servo ID:");Serial_AX.print(SERVO_ID);Serial_AX.println(" Voltage is out of set operating range ");
      break;

      case ANGLE_LIMIT_ERROR:
      Serial_AX.print("Servo ID:");Serial_AX.print(SERVO_ID);Serial_AX.println(" Goal postion is out of set operating range ");
      break;

      case OVER_HEAT_ERROR:
      Serial_AX.print("Servo ID:");Serial_AX.print(SERVO_ID);Serial_AX.println(" Interal temperature is out of set operating range ");
      break;

      case COMMAND_OUT_OFF_RANGE:
      Serial_AX.print("Servo ID:");Serial_AX.print(SERVO_ID);Serial_AX.println(" Command sent is incorrect ");
      break;

      case TX_CHECKSUM_ERROR:
      Serial_AX.print("Servo ID:");Serial_AX.print(SERVO_ID);Serial_AX.println(" Packet recived by Dynamixel had checksum error ");
      break;

      case OVERLOAD_ERROR:
      Serial_AX.print("Servo ID:");Serial_AX.print(SERVO_ID);Serial_AX.println(" Load on Dynamixel is greater then torque set ");
      break;

      case INSTRUCTION_ERROR:
      Serial_AX.print("Servo ID:");Serial_AX.print(SERVO_ID);Serial_AX.println(" Instruction sent is undefined ");
      break;

      case RX_ERROR:
      Serial_AX.print("Servo ID:");Serial_AX.print(SERVO_ID);Serial_AX.println(" NO! packet recived from Dynamixel ");
      break;

      default:
      Serial_AX.println(" Switch ERROR!");
