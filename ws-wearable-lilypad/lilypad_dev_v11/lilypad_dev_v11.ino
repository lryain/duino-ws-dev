...
void loop()
{
  tempSensorNew = analogRead(tempSensorPin);
  //Buzzer
  ...
  else  // If the button is not pressed go in here
  {
    ...
    tempSensorOld = analogRead(tempSensorPin);
    
    ///TEMP TEST 
    if(abs(tempSensorOld - tempSensorNew) <= 2)
    {
      //if the tech puts a little hot air over the temp sensor it will raise to at least 170
      digitalWrite(ledPins[7], LOW); //Turn on red LED - to indicate "hot"
      digitalWrite(ledPins[5], HIGH);
      digitalWrite(ledPins[6], HIGH);
    }
    else if(abs(tempSensorOld - tempSensorNew) >= 2 && abs(tempSensorOld - tempSensorNew) <= 5){
      digitalWrite(ledPins[6], LOW); //Turn on blue LED - to indicate "hot"
      digitalWrite(ledPins[5], HIGH);  // turn the red LED off
      digitalWrite(ledPins[7], HIGH); // turn the green LED off
    }
    else if(abs(tempSensorOld - tempSensorNew) >= 5){
      digitalWrite(ledPins[5], LOW); //Turn on red LED - to indicate "hot"
      digitalWrite(ledPins[6], HIGH); // turn the blue LED off
      digitalWrite(ledPins[7], HIGH); // turn the green LED off
    }
